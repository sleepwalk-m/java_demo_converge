...
import com.mask.puppeteer.pojo.Item;
...
public class JdPageProcessor implements PageProcessor {
    ...
    private static final String PAGE_URL = "https://search.jd.com/Search?keyword=%E6%89%8B%E6%9C%BA&suggest=1.his.0.0&wq=%E6%89%8B%E6%9C%BA&pvid=85ba37fdcea94716ae868e6957671f1b&page={page}&s=56&click=0";
    ...
    private void praseDetail(Page page) {
        ...
        if (StringUtils.isBlank(priceStr)){
            priceStr = html.$("div.p-price > input").nodes().get(0).xpath("///@data-jp").get();
        }
        if (StringUtils.isBlank(priceStr)){
            priceStr = "999.0";
        }
    ...
    private void parseList(Page page) {
        ...
        // 同时还要去做分页,这里做演示，只分页到第2页
        String url = PAGE_URL.replace("{page}","3");
        Request request = new Request(url);
        request.putExtra("level", "list");
        request.putExtra("pageNum", "3");
        page.addTargetRequest(request);
